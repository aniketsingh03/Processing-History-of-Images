...
def trainNet(net, batch_size, optimizer, start_epoch, n_epochs, learning_rate):
	...
	Ctr_dataset = Dataset(get_Ctr(QF) ,transform=transformations)
	Ctr_loader = DataLoader(Ctr_dataset, batch_size = batch_size, shuffle = True, num_workers = 4)

	Cval_dataset = Dataset(get_Cval(QF) ,transform=transformations)
	...
	for epoch in range(start_epoch, n_epochs):
		...
		val_loss = total_val_loss / len(Cval_loader)
		...
		with open(TRAIN_STATS_PHASE_1_FILENAME, 'a') as logfile:
...
PATH = QF+'/checkpoints.pth'
BEST_MODEL_PATH = QF+'/best_model_phase_1.pth'
TRAIN_STATS_PHASE_1_FILENAME = QF+'/stats_phase_1.log'
